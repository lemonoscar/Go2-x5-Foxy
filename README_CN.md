...
> 本副本 `rl_ras_n` 为 **ROS2 Foxy 专用版本**。

## `rl_ras_n` 改动说明（相对原 `rl_sar`）

本目录是从 `/home/lemon/Issac/rl_sar` 复制后改造的 Foxy 专用副本，原目录保持不变。

已完成的关键改动：

- 构建链路改为 **Foxy-only**：
  - `build.sh` 仅接受 `ROS_DISTRO=foxy`，否则直接报错退出。
  - ROS 构建固定使用 `colcon build --merge-install --symlink-install`。
  - 双 `package.xml` 软链接固定选择 `package.ros2.xml`。
- CMake 改为 **Foxy-only**：
...
- `robot_joint_controller` 代码层移除 Humble 条件分支，收敛为 Foxy 路径：
  - `src/robot_joint_controller/ros2/include/*.hpp`
  - `src/robot_joint_controller/ros2/src/*.cpp`

## `rl_ras_n` 使用方法（Foxy）

> 注意：以下命令全部在 `rl_ras_n` 目录执行，不要在原 `rl_sar` 执行。
> 下文原始上游文档仍保留了 Noetic/Humble 说明，仅作历史参考；本副本请以上述 Foxy 步骤为准。

1. 进入 Foxy 环境

```bash
source /opt/ros/foxy/setup.bash
echo $ROS_DISTRO   # 需要输出 foxy
```
...
source /opt/ros/foxy/setup.bash
source /home/lemon/Issac/rl_ras_n/install/setup.bash
ros2 run rl_sar rl_real_go2_x5 <YOUR_NETWORK_INTERFACE>
```

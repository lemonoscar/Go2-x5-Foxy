...
class BridgeBackend final : public rl_sar::adapters::arx::IArxBackend
{
public:
    explicit BridgeBackend(ArxAdapter::Config config)
        : config_(std::move(config)),
          state_timeout_ns_(
              static_cast<uint64_t>(std::max(0.0, config_.arm_state_timeout_ms) * 1'000'000.0))
    {
    ...
    bool Initialize(std::string* error) override
    {
        ...
        if (!MakeIpv4Endpoint(this->config_.bridge_host, this->config_.bridge_command_port, &cmd_addr))
        {
            if (error)
            {
                *error = "invalid bridge command endpoint";
            }
            return false;
        }

        if (this->config_.bridge_state_port <= 0 || this->config_.bridge_state_port > 65535)
        {
            if (error)
            {
                *error = "invalid bridge state port";
            }
            return false;
        }
        ...
        if (cmd_fd < 0)
        {
            if (error)
            {
                *error = "bridge command socket create failed: " + std::string(std::strerror(errno));
            }
            return false;
        }
        ...
        if (state_fd < 0)
        {
            if (error)
            {
                *error = "bridge state socket create failed: " + std::string(std::strerror(errno));
            }
            close(cmd_fd);
            return false;
        }
        ...
        bind_addr.sin_port = htons(static_cast<uint16_t>(this->config_.bridge_state_port));
        bind_addr.sin_addr.s_addr = htonl(INADDR_ANY);
        if (bind(state_fd, reinterpret_cast<sockaddr*>(&bind_addr), sizeof(bind_addr)) != 0)
        {
            if (error)
            {
                *error = "bridge state bind failed on port " +
                    std::to_string(this->config_.bridge_state_port) + ": " +
        ...
        if (connect(cmd_fd, reinterpret_cast<sockaddr*>(&cmd_addr), sizeof(cmd_addr)) != 0)
        {
            if (error)
            {
                *error = "bridge command connect failed to " + this->config_.bridge_host + ":" +
                    std::to_string(this->config_.bridge_command_port) + ": " +
                    std::string(std::strerror(errno));
            }
            close(cmd_fd);
            close(state_fd);
        ...
        this->cmd_socket_fd_ = cmd_fd;
        this->state_socket_fd_ = state_fd;
        this->healthy_ = false;
        this->last_update_ns_ = 0;
        return true;
#else
        if (error)
        {
            *error = "bridge backend requires Linux UDP sockets";
        }
        return false;
#endif
    }
    ...
    {
#if defined(__linux__)
        if (this->cmd_socket_fd_ >= 0)
        {
            close(this->cmd_socket_fd_);
            this->cmd_socket_fd_ = -1;
        }
        if (this->state_socket_fd_ >= 0)
        {
            close(this->state_socket_fd_);
            this->state_socket_fd_ = -1;
        }
#endif
        this->healthy_ = false;
    }
    ...
    bool SendCommand(const ArmCommandFrame& cmd, std::string* error) override
    {
        ...
        if (this->cmd_socket_fd_ < 0)
        {
            if (error)
            {
                *error = "bridge backend is not initialized";
            }
            return false;
        }

        const auto bytes = SerializeTypedArmCommandFrame(cmd);
        const ssize_t sent = send(this->cmd_socket_fd_, bytes.data(), bytes.size(), 0);
        if (sent < 0)
        {
            this->healthy_ = false;
            if (error)
            {
                *error = "bridge command send failed: " + std::string(std::strerror(errno));
            }
            return false;
        }
        if (static_cast<size_t>(sent) != bytes.size())
        {
            ...
            if (error)
            {
                *error = "bridge command send was partial";
            }
            return false;
        }
        this->healthy_ = true;
    ...
    {
#if defined(__linux__)
        if (this->state_socket_fd_ < 0 || !state)
        {
            if (error)
            {
                *error = "bridge backend is not initialized";
        ...
        const ssize_t received =
            recv(this->state_socket_fd_, buffer.data(), buffer.size(), 0);
        if (received < 0)
        {
            ...
            if (error)
            {
                *error = "bridge state recv failed: " + std::string(std::strerror(errno));
            }
            return false;
        }

        std::vector<uint8_t> bytes(buffer.begin(), buffer.begin() + received);
        ArmStateFrame parsed;
        std::string parse_error;
        if (ParseTypedArmStateFrame(bytes, &parsed, &parse_error))
        {
            this->last_update_ns_ = GetMonotonicNsNow();
            this->healthy_ = true;
            parsed.backend_mode = static_cast<uint16_t>(ArxAdapter::BackendType::Bridge);
            *state = parsed;
            return true;
        }

        Go2X5IPC::ArmStatePacket legacy_packet;
        if (!Go2X5IPC::ParseStatePacket(bytes, legacy_packet, &parse_error))
        {
            this->healthy_ = false;
            if (error)
            {
        ...
        *state = ArmStateFrame{};
        ...
        state->header.source_monotonic_ns = GetMonotonicNsNow();
        ...
        if (legacy_packet.state_from_backend)
        {
        ...
        state->joint_count = std::min<uint16_t>(legacy_packet.joint_count, kArxJointCount);
        state->backend_mode = static_cast<uint16_t>(ArxAdapter::BackendType::Bridge);
        for (size_t i = 0; i < std::min<size_t>(kArxJointCount, legacy_packet.q.size()); ++i)
        {
            state->q[i] = legacy_packet.q[i];
        }
        for (size_t i = 0; i < std::min<size_t>(kArxJointCount, legacy_packet.dq.size()); ++i)
        {
            state->dq[i] = legacy_packet.dq[i];
        }
        for (size_t i = 0; i < std::min<size_t>(kArxJointCount, legacy_packet.tau.size()); ++i)
        {
            state->tau[i] = legacy_packet.tau[i];
        }
        this->last_update_ns_ = GetMonotonicNsNow();
        this->healthy_ = true;
        return true;
    ...
    bool IsHealthy() const override
    {
        ...
        if (this->state_timeout_ns_ == 0 || this->last_update_ns_ == 0)
        {
            return this->healthy_;
        }
        const uint64_t age_ns = GetMonotonicNsNow() - this->last_update_ns_;
        return age_ns <= this->state_timeout_ns_;
    }
...
bool ArxAdapter::InitializeBackend(std::string* error)
{
    ...
    auto try_backend =
        [&](BackendType type, std::unique_ptr<arx::IArxBackend> candidate, std::string* backend_error) -> bool
    {
        if (!candidate)
        {
            if (backend_error)
            {
                *backend_error = "backend factory returned null";
            }
            return false;
        }
        if (!candidate->Initialize(backend_error))
        {
            return false;
        }
    ...
    std::string preferred_error;
    if (config_.preferred_backend == BackendType::Bridge)
    {
        if (try_backend(
                BackendType::Bridge,
                std::make_unique<BridgeBackend>(config_),
                &preferred_error))
        {
            return true;
        }
    ...
    {
        if (try_backend(
                BackendType::InProcessSdk,
                std::make_unique<SdkBackend>(config_),
                &preferred_error))
        {
            return true;
        }
    ...
    if (config_.allow_fallback_to_bridge && config_.preferred_backend != BackendType::Bridge)
    {
        if (try_backend(
                BackendType::Bridge,
                std::make_unique<BridgeBackend>(config_),
                &fallback_error))
        {
            LogWarning("Preferred SDK backend unavailable, using bridge fallback");
            return true;
        }
    }
    ...
    if (error)
    {
        if (!preferred_error.empty() && !fallback_error.empty())
        {
            *error = "preferred backend failed: " + preferred_error +
                "; bridge fallback failed: " + fallback_error;
        }
        else if (!preferred_error.empty())
        {
